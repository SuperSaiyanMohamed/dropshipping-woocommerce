...
class Knawat_Dropshipping_WC_MP_Orders {
	...
	public function knawat_format_order( $order_id, $is_update = false, $json = true ) {
		...
		$order_whitelist_fields_flipped = array_flip($order_whitelist_fields);
		$item_whitelist_fields_flipped = array_flip($item_whitelist_fields);
		$new_order = array();
		...
		foreach ( $order as $key => $value ) {
			if (isset($order_whitelist_fields_flipped[$key])){
		...
		if ( isset( $new_order['items'] ) && ! empty( $new_order['items'] ) ) {
			foreach ( $new_order['items'] as $itemkey => $item ) {
				...
				foreach ( $item as $ikey => $ivalue ) {
					if ( ! isset( $item_whitelist_fields_flipped[$ikey] ) ) {
